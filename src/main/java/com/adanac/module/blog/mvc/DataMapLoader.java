...
public abstract class DataMapLoader {
	...
	public static Map<String, DataMap> load() {
		...
		File[] files = Configuration.getClasspathFile("com/adanac/module/blog/dynamic").listFiles();
		for (int i = 0; i < files.length; i++) {
			...
			try {
				Class<?> clazz = Configuration.getClassLoader().loadClass("com.adanac.module.blog.dynamic." + fileName);
