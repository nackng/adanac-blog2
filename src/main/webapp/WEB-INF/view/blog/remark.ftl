<table align="center" border="0" cellpadding="3" cellspacing="1" width="100%">
    <tbody>
    <tr valign="bottom">
        <td align="center">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody>
                <tr>
                    <td height="100" align="center" valign="bottom">
                        <span id="good_times_span">${article.good_times?default("0")}</span><br>
                        <img id="good_times_img" src="${resRoot}/img/common/remark_bg_2.gif"
                             height="${article.good_times_height?default("0")}" border="0" width="20">
                    </td>
                </tr>
                <tr>
                    <td height="42" align="center">
                        <img src="${resRoot}/img/common/remark_good.gif" height="32" border="0" width="32">
                    </td>
                </tr>
                <tr>
                    <td height="20" align="center" valign="top">精彩</td>
                </tr>
                <tr>
                    <td align="center">
                        <input name="column" value="good_times" type="radio">
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
        <td align="center">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody>
                <tr>
                    <td height="100" align="center" valign="bottom">
                        <span id="touch_times_span">${article.touch_times?default("0")}</span><br>
                        <img id="touch_times_img" src="${resRoot}/img/common/remark_bg_2.gif"
                             height="${article.touch_times_height?default("0")}" border="0" width="20">
                    </td>
                </tr>
                <tr>
                    <td height="42" align="center">
                        <img src="${resRoot}/img/common/remark_touch.gif" height="32" border="0" width="32">
                    </td>
                </tr>
                <tr>
                    <td height="20" align="center" valign="top">
                        感动
                    </td>
                </tr>
                <tr>
                    <td align="center">
                        <input name="column" value="touch_times" type="radio">
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
        <td align="center">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody>
                <tr>
                    <td height="100" align="center" valign="bottom">
                        <span id="funny_times_span">${article.funny_times?default("0")}</span><br>
                        <img id="funny_times_img" src="${resRoot}/img/common/remark_bg_1.gif"
                             height="${article.funny_times_height?default("0")}" border="0" width="20">
                    </td>
                </tr>
                <tr>
                    <td height="42" align="center">
                        <img src="${resRoot}/img/common/remark_funny.gif" height="32" border="0" width="32">
                    </td>
                </tr>
                <tr>
                    <td height="20" align="center" valign="top">
                        搞笑
                    </td>
                </tr>
                <tr>
                    <td align="center">
                        <input name="column" value="funny_times" type="radio">
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
        <td align="center">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody>
                <tr>
                    <td height="100" align="center" valign="bottom">
                        <span id="happy_times_span">${article.happy_times?default("0")}</span><br>
                        <img id="happy_times_img" src="${resRoot}/img/common/remark_bg_2.gif"
                             height="${article.happy_times_height?default("0")}" border="0" width="20">
                    </td>
                </tr>
                <tr>
                    <td height="42" align="center">
                        <img src="${resRoot}/img/common/remark_happy.gif" height="32" border="0" width="32">
                    </td>
                </tr>
                <tr>
                    <td height="20" align="center" valign="top">
                        开心
                    </td>
                </tr>
                <tr>
                    <td align="center">
                        <input name="column" value="happy_times" type="radio">
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
        <td align="center">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody>
                <tr>
                    <td height="100" align="center" valign="bottom">
                        <span id="anger_times_span">${article.anger_times?default("0")}</span><br>
                        <img id="anger_times_img" src="${resRoot}/img/common/remark_bg_1.gif"
                             height="${article.anger_times_height?default("0")}" border="0" width="20">
                    </td>
                </tr>
                <tr>
                    <td height="42" align="center">
                        <img src="${resRoot}/img/common/remark_anger.gif" height="32" border="0" width="32">
                    </td>
                </tr>
                <tr>
                    <td height="20" align="center" valign="top">
                        愤怒
                    </td>
                </tr>
                <tr>
                    <td align="center">
                        <input name="column" value="anger_times" type="radio">
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
        <td align="center">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody>
                <tr>
                    <td height="100" align="center" valign="bottom">
                        <span id="bored_times_span">${article.bored_times?default("0")}</span><br>
                        <img id="bored_times_img" src="${resRoot}/img/common/remark_bg_1.gif"
                             height="${article.bored_times_height?default("0")}" border="0" width="20">
                    </td>
                </tr>
                <tr>
                    <td height="42" align="center">
                        <img src="${resRoot}/img/common/remark_bored.gif" height="32" border="0" width="32">
                    </td>
                </tr>
                <tr>
                    <td height="20" align="center" valign="top">
                        无聊
                    </td>
                </tr>
                <tr>
                    <td align="center">
                        <input name="column" value="bored_times" type="radio">
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
        <td align="center">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody>
                <tr>
                    <td height="100" align="center" valign="bottom">
                        <span id="water_times_span">${article.water_times?default("0")}</span><br>
                        <img id="water_times_img" src="${resRoot}/img/common/remark_bg_1.gif"
                             height="${article.water_times_height?default("0")}" border="0" width="20">
                    </td>
                </tr>
                <tr>
                    <td height="42" align="center">
                        <img src="${resRoot}/img/common/remark_water.gif" height="32" border="0" width="32">
                    </td>
                </tr>
                <tr>
                    <td height="20" align="center" valign="top">灌水</td>
                </tr>
                <tr>
                    <td align="center">
                        <input name="column" value="water_times" type="radio">
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
        <td align="center">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody>
                <tr>
                    <td height="100" align="center" valign="bottom">
                        <span id="surprise_times_span">${article.surprise_times?default("0")}</span><br>
                        <img id="surprise_times_img" src="${resRoot}/img/common/remark_bg_1.gif"
                             height="${article.surprise_times_height?default("0")}" border="0" width="20">
                    </td>
                </tr>
                <tr>
                    <td height="42" align="center">
                        <img src="${resRoot}/img/common/remark_surprise.gif" height="32" border="0" width="32">
                    </td>
                </tr>
                <tr>
                    <td height="20" align="center" valign="top">
                        惊讶
                    </td>
                </tr>
                <tr>
                    <td align="center">
                        <input name="column" value="surprise_times" type="radio">
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
    </tr>
    </tbody>
</table>
